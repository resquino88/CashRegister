...
public class RuleEngine(IEnumerable<IRule<string, BasicRuleInfoDTO>> rules, IRuleRepository ruleRepository, IOptions<RuleEngineInfo> options) : IRuleEngine
{
    ...
    public async Task<string> Start(BasicRuleInfoDTO info)
    {
        ...
        var defaultRule = rules.SingleOrDefault(r => r.Name() == _defaultRuleName && foundActiveRules.Any(far => far.Name == r.Name())) ?? throw new InvalidOperationException($"No active default rule found with name '{_defaultRuleName}'. Ensure the rule is seeded and marked active in the database.");
