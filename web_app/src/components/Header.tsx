...
export default function Header({
  ...
  const setSelectedCurrency = (selectedCurrencyId: number) => {
    if (headerData === undefined || headerData.selectedCountry === undefined)
      return;

    const selectedCountry = headerData.selectedCountry;
    const foundCurrency = selectedCountry.currencies.find(
    ...
    >
      <Toolbar sx={{ gap: 2, flexWrap: "wrap", py: { xs: 1, sm: 0 } }}>
        ...
        <FormControl size="small" sx={{ minWidth: 200 }}>
          ...
          <Select
            value={headerData?.selectedCountry?.id ?? ""}
            label="Country"
            onChange={(e) => setSelectedCountry(Number(e.target.value))}
          >
            {countries.map((c) => (
              <MenuItem key={c.id} value={c.id}>
                {c.name} ({c.abbrevation})
              </MenuItem>
            ))}
          </Select>
        </FormControl>
        ...
        <FormControl size="small" sx={{ minWidth: 120 }} disabled={!headerData}>
          ...
          <Select
            value={headerData?.selectedCurrency?.id ?? ""}
            label="Currency"
