...
export default function UploadInfo({ decimalSeparator }: UploadInfoProps) {
  ...
  const lines = [
    `2${decimalSeparator}13,3${decimalSeparator}00`,
    `1${decimalSeparator}97,2${decimalSeparator}00`,
    `3${decimalSeparator}33,5${decimalSeparator}00`,
  ];
      ...
      <List dense disablePadding>
        {notes.map((note, i) => (
          <ListItem
            key={i}
            disableGutters
            sx={{ py: 0.3, alignItems: "center" }}
          >
            <ListItemIcon sx={{ minWidth: 24 }}>
              <FiberManualRecordIcon
                sx={{ fontSize: 7, color: colors.bullet }}
              />
            </ListItemIcon>
            <Typography variant="body2" color={colors.noteText}>
              {note}
            </Typography>
          </ListItem>
        ))}
