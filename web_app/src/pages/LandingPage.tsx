  ...
  const initiateFileUpload = async (file: File) => {
    ...
    try {
      const result = await uploadFile(file, {
        countryId: headerData.selectedCountry.id,
        currencyId: headerData.selectedCurrency.id,
      });
  ...
  return (
    <>
      {isLoading ? (
        ...
        <ErrorScreen message={errorMessage} onRetry={errorRetry} />
      ) : (
        <>
          <Header
            countries={countries}
          ...
          <Container maxWidth="lg" sx={{ py: 4 }}>
            <Stack spacing={3}>
              <UploadInfo
                decimalSeparator={
                  headerData?.selectedCurrency?.currencySeparator ?? "."
                }
              />
              <UploadArea
                file={file}
                setFile={setFile}
                selectedCurrency={headerData?.selectedCurrency}
                onFileUpload={initiateFileUpload}
              />
            </Stack>
          </Container>
        </>
